...
export default function MyLeave() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setMsg(t('leave.submitted'));
      setForm(f => ({ ...f, start_date: '', end_date: '', reason: '' }));
      load();
    } catch (e: any) {
      setError(e.response?.data?.detail || t('leave.submitFailed'));
    }
  };
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-800 mb-6">🏖️ {t('leave.title')}</h1>
        ...
        {balances.map(b => (
          <div key={b.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            ...
            <div className="text-xs text-gray-400 space-y-0.5">
              <div>{t('leave.entitled')}: {b.entitled}</div>
              <div>{t('leave.used')}: {b.used}</div>
              <div>{t('leave.pending')}: {b.pending}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-8">
        <h2 className="font-semibold text-gray-700 mb-4">{t('leave.newRequest')}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <input type="date" value={form.start_date} onChange={e => setForm({ ...form, start_date: e.target.value })}
            className="px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder={t('common.start')} />
          <input type="date" value={form.end_date} onChange={e => setForm({ ...form, end_date: e.target.value })}
            className="px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder={t('common.end')} />
          <input value={form.reason} onChange={e => setForm({ ...form, reason: e.target.value })}
            className="px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder={t('leave.optionalReason')} />
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100">
          <h2 className="font-semibold text-gray-700">{t('leave.requestHistory')}</h2>
        </div>
          ...
          <tbody>
            {requests.map(r => (
              <tr key={r.id} className="border-b border-gray-50">
                ...
                <td className="px-6 py-3">
                  {r.status === 'PENDING' && (
                    <button onClick={() => cancel(r.id)} className="text-red-600 hover:underline text-xs">{t('leave.cancelRequest')}</button>
                  )}
                </td>
              </tr>
            ))}
            {requests.length === 0 && (
              <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-400">{t('leave.noRequests')}</td></tr>
            )}
          </tbody>
        </table>
      </div>
